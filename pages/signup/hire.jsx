...
export default function Hire() {
    ...
    return (<>
        <Layout>
            <form action="" className='text-primary-900 w-[320px] md:w-[450px] mx-auto lg:w-2/4' onSubmit={handleSubmit}>
                <h1 className="font-[500] text-[40px] leading-[60px]">Hello,</h1>
                <div className="flex flex-col gap-[35px] mt-[36px]">
                    <p className="text-[20px]">Please register to continue to <span className='font-bold hover:text-primary-100'>hire</span> or click here to <Link href="/signup/work" className='font-bold hover:text-primary-100'>REGISTER TO WORK</Link></p>

                    <input required
                        onChange={(e) => { setUserDetails({ ...userDetails, fname: e.target.value }) }}
                        type="text" className="w-full px-[17px] py-[25px] border border-[#3D3D3D] rounded-[10px] text-[20px]" placeholder='Full name' />

                    <input required
                        onChange={(e) => { setUserDetails({ ...userDetails, email: e.target.value }) }}
                        type="email" className="w-full px-[17px] py-[25px] border border-[#3D3D3D] rounded-[10px] text-[20px]" placeholder='Email' />

                    <input required
                        onChange={(e) => { setUserDetails({ ...userDetails, stateOfResidence: e.target.value }) }}
                        type="text" className="w-full px-[17px] py-[25px] border border-[#3D3D3D] rounded-[10px] text-[20px]" placeholder='State of residence' />

                    <input required
                        onChange={(e) => { setUserDetails({ ...userDetails, phone: e.target.value }) }}
                        type="phone" className="w-full px-[17px] py-[25px] border border-[#3D3D3D] rounded-[10px] text-[20px]" placeholder='Phone number ' />

                    <div className="relative w-[4full">
                        <input required
                    ...
                    <button type='submit' className="w-full py-[13px] border border-[#3D3D3D] font-[500] text-center rounded-[10px] text-[36px] text-black">Register</button>
